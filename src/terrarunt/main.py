...
from .cleaner import CleanOperations
...
Examples:
  ...
  terrarunt list-stacks
    ...
    clean_parser = subparsers.add_parser("clean", help="Delete Terraform-generated files for a single stack")
    clean_parser.add_argument("--stack", required=True, help="Stack name")

    clean_all_parser = subparsers.add_parser("clean-all", help="Clean all stacks")
    clean_all_parser.add_argument("--confirm", action="store_true", help="Skip confirmation prompt")

    return parser

def handle_single_clean_command(args, operations: CleanOperations) -> int:
    """Handle single stack clean command"""
    try:
        success = operations.clean_stack(args.stacks)
...
def handle_clean_all_command(args, operations: CleanOperations) -> int:
    """Handle clean-all command"""
    try:
        if not args.confirm:
            stacks = stack_manager.discover_stacks()
            
            print(f"This will clean {len(stacks)} stacks and remove local state files:")
        ...
        success = operations.clean_all()
        return 0 if success else 1
        
    except Exception as e:
        logger.error(f"Clean-all error: {e}")
        return 1
...
def handle_single_stack_command(args, operations: StackOperations) -> int:
    ...
    try:
        ...
        if args.command in ["plan", "apply", "destroy", "init"]:
            success = command_func(args.env, args.stack, args.tf_args)
        else:
            # TODO: Should be unreachable but check.
...
def handle_graph_command() -> int:
    ...
    try:
        ...
        for i, stack in enumerate(ordered_stacks, 1):
            ...
            deps_info = f" <- {', '.join(stack.dependencies['paths'])}" if stack.dependencies.get('paths') else ""
...
def main() -> int:
    ...
    try:
        if args.command in ["init", "plan", "apply", "destroy"]:
            ...
            operations = CleanOperations(args.dry_run)
            result = handle_single_clean_command(args, operations)
            
            if args.dry_run:
                operations.cleaner.show_summary()
            
            return result

        elif args.command == "clean-all":
            operations = CleanOperations(args.dry_run)
            result = handle_clean_all_command(args, operations)
